...
from pyclaw.bus.events import InboundMessage
...
class SubagentManager:
    ...
    async def spawn(
        ...
        Args:
            ...
            origin_sender: 来源连接 ID（Web UI 场景为 websocket client_id），
                用于把结果路由回发起请求的连接；非 websocket 来源为 None。
    ...
    async def _announce_result(
        ...
        # Web UI 来源：sender_id 携带 websocket client_id，供 _process_system_message
        # 把结果路由回发起请求的那个连接（channel=client_id）。
        msg = InboundMessage(
            channel="system",
            sender_id=origin.get("sender") or "subagent",
